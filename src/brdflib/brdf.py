...
def build_ndf_kernel(frC, n_theta, n_phi, isotropic):
    ...
    if isotropic:
        ...
        omega = np.column_stack(
            (np.sin(theta), np.zeros(n_theta), np.cos(theta))
        )
        ...
        for j in range(N):
            ...
            integral = trapezoid(tmp, dx=dphi, axis=1)
        ...
        for j in range(N):
            # Calculate kernel
            K[:, j] = (
                weights[j] * frC * np.dot(omega_m, omega_m[j]).clip(0, None)
            )
    return K
    ...
    if isotropic:
        ...
        for j in range(N):
            if omega_m[j, 2] > EPSILON:
                ...
                K_ = (
                    frC
                    * np.dot(omega_m, omega_m[j]).clip(0, None)
...
def beckmann_parameters(P_in, isotropic):
    ...
    if isotropic:
        ...
        x_n = E[0]
        y_n = E[1]
        a_x = np.sqrt(2.0 * (E[2] - np.power(x_n, 2)))
        a_y = np.sqrt(2.0 * (E[3] - np.power(y_n, 2)))
        rho = 2.0 * (E[4] - x_n * y_n) / (a_x * a_y)
...
def spherical2cartesian(theta, phi):
    ...
    [sin_theta, cos_theta] = drjit.sincos(drjit.llvm.ad.Float32(theta))
    [sin_phi, cos_phi] = drjit.sincos(drjit.llvm.ad.Float32(phi))
...
def elevation(d):
    dist = drjit.sqrt(drjit.sqr(d.x) + drjit.sqr(d.y) + drjit.sqr(d.z - 1.0))
    ...
    for i in range(Dvis.shape[0]):  # incident elevation
        for j in range(Dvis.shape[1]):  # incident azimuth
            ...
            for k in range(Dvis.shape[2]):  # observation azimuth
                ...
                # Bidirectional NDF
                Dvis[i, j, k] = (
                    drjit.max([0, drjit.dot(omega, omega_i)]) * D_m / sigma_i
                )
    return Dvis
...
def vndf_intp2sample(Dvis_intp):
    ...
    for l in range(n_theta):
        ...
        Dvis_sampler[:, :, :, l] = (
            Dvis_intp[:, :, :, l] * jc / (n_theta * n_theta)
        )
    return Dvis_sampler
...
def outgoing_direction(
    ...
    for i in range(phi_i.size):
        for j in range(theta_i.size):
            ...
            act &= drjit.cuda.ad.Bool(
                np.invert(inv)
            )  # above surface plane  # above surface plane
...
def weight_measurements(
    spec, D, sigma, theta_i, phi_i, theta_o, phi_o, active=None
):
    MarginalContinuous2D0 = mi.MarginalContinuous2D0
    Vector2f = mi.Vector2f
    ...
    for i in range(phi_i.size):
        for j in range(theta_i.size):
            ...
            # Outgoing direction
            wo = spherical2cartesian(
                theta_o[i, j].flatten(), phi_o[i, j].flatten()
            )
            # Phase direction
